...
//  Awasthi027
//
//  Created by Ashish Awasthi on 08/12/21.
//

import Foundation
import UIKit

/// calculate text size on label
extension UILabel {
    ...
    func countLines(width: CGFloat = .greatestFiniteMagnitude, height: CGFloat = .greatestFiniteMagnitude) -> Int {
        ...
        let rect = CGSize(width: width, height: height)
        let labelSize = myText.boundingRect(with: rect, options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: self.font  ?? UIFont.systemFont(ofSize: 14.0)], context: nil)
